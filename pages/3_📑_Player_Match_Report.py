...
st.markdown("""<p style="font-family:Futura; color:white; font-size: 40px;">PLAYER MATCH REPORT</p>""",unsafe_allow_html=True)

st.markdown(
    """
    <p style="font-family:Gill Sans; color:white; font-size: 18px;">
    Create Player Match Data Report for a specific match in the English Premier League
    
    <p style="font-family:Gill Sans; color:white; font-size: 18px;">Report includes Touch Map,Pass Map,Shot Map and Ball Recovery Map
    
    <p style="font-family:Gill Sans; color:white; font-size: 18px;">Set attacker or defender to generate different report types
    
    <p style="font-family:Gill Sans; color:Red; font-size: 18px;">Note : If python scalar error shows up then the player you have selected probably didn't play the match

    <p style="font-family:Gill Sans; color:white; font-size: 18px;">Instructions : </p>
    
    1. Go to [Whoscored.com](https://www.whoscored.com/Regions/252/Tournaments/2/Seasons/9075/Stages/20934/Show/England-Premier-League-2022-2023) and copy link address of your desired match
    
    2. Paste it in enter match url bar
    
    3. Select player name and position and view the report
...
def scrape_and_process_data(url):
    driver = webdriver.Chrome('/Users/ligandrosy/Documents/VSCODE Folders/Liga-Football/chromedriver')
    driver.get(url)
...
# Main Streamlit code
url = st.text_input("Enter match URL:") + "#chalkboard"
url = url.replace("Show", "Live")

if len(url) > 30:
    data = scrape_and_process_data(url)
    ...
    col_list = [x for x in player_df.Name.values]
    ...
    status1 =["Attacker","Defender"]
    status = st.radio("Select Position",horizontal=True,options = status1)
